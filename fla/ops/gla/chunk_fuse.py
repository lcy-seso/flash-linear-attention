...
def fused_chunk_gla_fwd_kernel(
    ...
    for i in range(0, tl.cdiv(T, BT)):
        ...
        if CHECK and i == 0:
            b_o = tl.dot(
                b_q.to(b_v.dtype), b_h.to(b_v.dtype), allow_tf32=False)
    ...
    if STORE_FINAL_STATE:
        p_final = tl.make_block_ptr(final_state + i_bh * DK * DV, (DK, DV),
                                    ...
                                    (1, 0))
        tl.store(
            p_final, b_h.to(p_final.dtype.element_ty), boundary_check=(0, 1))
...
def fused_chunk_gla_bwd_kernel(
    ...
    for i in range(1, tl.cdiv(T, BT) + 1):
        ...
        p_dk = tl.make_block_ptr(dk + (i_bh + i_v * B * H) * s_qk_h, (T, DK),
                                 (s_qk_t, s_qk_d), (T - i * BT, i_k * BK),
                                 (BT, BK), (1, 0))
        p_dv = tl.make_block_ptr(dv + (i_bh + i_k * B * H) * s_vo_h, (T, DV),
                                 (s_vo_t, s_vo_d), (T - i * BT, i_v * BV),
                                 (BT, BV), (1, 0))
        # [DK, BT]
        b_q = tl.load(p_q, boundary_check=(0, 1))
        # [BT, DK]
        ...
        if CHECK and i == 1:
            b_dk = tl.trans(
                tl.dot(b_dh.to(b_v.dtype), tl.trans(b_v), allow_tf32=False))
            b_dv = tl.dot(
                (b_k).to(b_v.dtype), b_dh.to(b_v.dtype), allow_tf32=False)
...
def fwd_inner_chunk(
    ...
    p_A = A + (i_bh + (i_k * B * H)) * (
        tl.cdiv(T, BT) * BT * BT) + i_t * BT * BT + tl.arange(0, BT)
...
class FusedChunkGLAFunction(torch.autograd.Function):
    ...
    def backward(ctx, do, d_final_state=None):
        ...
        return dq.to(q), dk.to(k), dv.to(v), dg.to(
            ctx.g_dtype), None, None, None
